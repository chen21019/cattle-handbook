...
description: "Documents auth-service and local/external auth risk."
audience:
  - human-maintainer
  - ai-agent
tags:
...
## Purpose
Documents auth-service and local/external auth risk. It should be updated whenever the related repository, dependency, or workflow changes.
...
圖名：Authentication And Authorization Flow
用途：說明 Authentication And Authorization 在 Rancher 1.6 維護知識中的位置。
