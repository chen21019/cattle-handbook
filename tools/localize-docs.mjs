...
const titles = new Map(Object.entries({
  ...
  'security/verified-risk-notes.md': '風險查證記錄',
  ...
  'runbooks/build-failure.md': '建置失敗處理手冊',
  'runbooks/dependency-upgrade.md': '依賴升級處理手冊',
  'runbooks/broken-api.md': 'API 異常處理手冊',
  'runbooks/ui-build-error.md': 'UI 建置錯誤處理手冊',
  'runbooks/docker-image-error.md': 'Docker 映像錯誤處理手冊',
  'runbooks/database-migration-error.md': '資料庫遷移錯誤處理手冊',
  'runbooks/release-failure.md': '發版失敗處理手冊',
...
const phraseReplacements = [
  ...
  ['Produce a task summary with scope, risk, verification, and rollback.', '先產出包含範圍、風險、驗證與回復方案的任務摘要。'],
  ['Prefer the smallest patch and avoid unrelated formatting changes.', '優先採用最小 patch，避免無關格式化變更。'],
  ['Keep source-backed risk notes intact.', '保留已查證的風險說明。'],
  ['Rancher 1.6 risk status must be verified against actual dependencies, images, CVEs, and official sources before being stated.', 'Rancher 1.6 的風險狀態必須以實際依賴、image、CVE 與官方來源查證後記錄。'],
  ...
  ['Do not remove source-backed security notes.', '不可移除已查證的 security note。'],
  ...
  ['Revert only your own changes, preserve user work, and document why rollback was needed.', '只回復自己的變更，保留使用者工作，並記錄需要回復方案的原因。'],
  ...
  ['Dependency Risk Matrix', '依賴風險矩陣'],
  ['Verified Risk Notes', '風險查證記錄'],
  ['For Human Maintainers Flow', '人類維護者流程'],
  ['For Human Maintainers 流程', '人類維護者流程'],
  ['把 For Human Maintainers 轉成可執行的維護流程。', '把人類維護者指南轉成可執行的維護流程。'],
  ['Use the smallest safe command sequence and avoid unrelated edits.', '使用最小安全指令序列，避免無關修改。'],
  ['Support status 警告不可移除', '已查證風險說明不可移除']
];
...
function zhDescription(rel, title) {
  ...
  const sectionMap = {
    ...
    security: '記錄需來源查證的安全、依賴、映像檔與營運風險。',
    'ai-guide': '提供 AI Agent 安全維護 Rancher 1.6 的操作契約。',
    runbooks: '提供問題排查、修補、驗證與回復方案的可操作處理手冊。',
