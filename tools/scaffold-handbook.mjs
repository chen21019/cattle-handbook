...
const riskRows = depRows.slice(0, 160).map((dep) => `| ${dep.name} | ${dep.ecosystem} | ${dep.repo} / ${dep.path} | ${dep.version} | TODO query upstream | ${dep.risk} | ${dep.risk === 'high' ? 'possible' : 'unknown'} | ${/auth|jwt|crypto|ssl|secret|mysql|mariadb|docker|net|spring|jetty/i.test(dep.name) ? 'yes' : 'review'} | no | likely | targeted build + regression test |`).join('\n');
...
const docs = [
  ...
  ['getting-started/for-human-maintainers', 'For Human Maintainers', 'A first-week learning path for engineers new to Rancher 1.6.'],
  ...
  ['dependency-map/index', 'Dependency Map', 'Dependency map entry point and maintenance rules.'],
  ...
  ['security/security-overview', 'Security Overview', 'Security posture for a legacy/EOL platform.'],
  ['security/known-eol-risks', 'Known EOL Risks', 'Explicit EOL and production-risk warnings.'],
  ['security/dependency-vulnerability-triage', 'Dependency Vulnerability Triage', 'CVE triage flow for old dependencies.'],
  ...
  ['runbooks/broken-api', 'Broken API', 'Runbook for API behavior regressions.'],
...
function frontmatter(title, description, slug) {
  ...
  const priority = /for-ai-agents|codex-cli|safe-editing|risk-matrix|patch-workflow|known-eol|dependency-upgrade|repository-map|search-architecture|search-index-maintenance/.test(slug) ? 'high' : 'normal';
...
const special = new Map([
  ...
  ['security/known-eol-risks', knownEolDoc()]
]);
...
write(path.join(root, 'docs-site', 'src', 'components', 'SearchBox.astro'), `<div class=\"search-panel\"><input aria-label=\"Search docs\" placeholder=\"Search errors, dependencies, APIs, runbooks, or AI tasks\" /></div>\n`);
